            ...
            <div className={viewMode === 'grid' ? 'space-y-1.5' : 'space-y-0.5'}>
              {[...recentVideos].sort((a, b) => {
                ...
                <button
                  ...
                  className={`group/card w-full flex items-center gap-3 px-4 py-2.5 rounded-xl bg-chalk-surface/30 border hover:bg-chalk-surface/50 hover:border-chalk-border/40 hover:scale-[1.02] hover:shadow-lg hover:shadow-black/20 transition-all duration-200 text-left relative overflow-hidden ${vidIdx === selectedVideoIdx ? 'border-chalk-accent/50 ring-1 ring-chalk-accent/30' : 'border-chalk-border/20'}`}
                >
                  {/* Watch progress bar */}
                  {watchPct > 0 && (
